...
def update_trade_history():
    ...
    If a trade is closed, logs its outcome to Firestore (and local CSV) for AI training.
    """
    ...
    from firebase_logger import update_trade_outcome, fetch_historical_trades
    closed_tickets = []
    ...
    for index, row in active_df.iterrows():
        ...
        if len(deals_for_position) >= 2:
            ...
            # Save outcome to Firestore and local CSV fallback
            update_trade_outcome(ticket, profit, win)
    ...
    full_history = fetch_historical_trades()
    if not full_history.empty and len(full_history) % config.RETRAIN_AFTER_N_TRADES == 0:
