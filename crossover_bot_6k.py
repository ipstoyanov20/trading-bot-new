...
# Stop Loss and Take Profit Settings (SL is 3x smaller than TP -> 1:3 Risk:Reward)
SL_PRICE_DIST = 2.0        # $2.00 Gold price move for Stop Loss
TP_PRICE_DIST = 6.0        # $6.00 Gold price move for Take Profit (3x SL)
...
def place_scalping_order(symbol, order_type):
    """
    Places an order for Gold (XAUUSD) with SL 3 times smaller than TP (1:3 Risk:Reward).
    """
    symbol_info = mt5.symbol_info(symbol)
    if not symbol_info:
