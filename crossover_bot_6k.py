...
def place_scalping_order(symbol, order_type):
    """Places an order with fixed lots and $20 Stop Loss for Gold."""
    symbol_info = mt5.symbol_info(symbol)
    if not symbol_info:
    ...
    # Calculate $20 Stop Loss distance for Gold
    ...
    if tick_value > 0 and tick_size > 0:
        ...
        sl_dist = 20.0 / (lots * 100.0) # Fallback for Gold
...
def monitor_profit_target(symbol, target_profit=40.0, stop_loss=20.0):
    """
    Monitors all open bot positions for the symbol.
    Closes the trade when profit reaches target_profit ($40) OR loss reaches stop_loss ($20).
