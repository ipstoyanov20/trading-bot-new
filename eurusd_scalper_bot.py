...
LOT_SIZE = 0.30            # Calibrated volume (0.30 lot = $3/pip move, ~$0.30 spread)
COOLDOWN_SECONDS = 60      # 1-minute cooldown after trade exit

# 1:3 Risk-to-Reward Settings (1 Win ($90) recovers 3 Losses ($30 each))
SL_PRICE_DIST = 0.00100    # 10 pips for Stop Loss
TP_PRICE_DIST = 0.00300    # 30 pips for Take Profit (3x SL)
...
def place_scalping_order(symbol, order_type):
    """
    Places an order for EURUSD with 1:3 RR (SL 10 pips / TP 30 pips).
    """
    symbol_info = mt5.symbol_info(symbol)
    if not symbol_info:
...
def monitor_positions(symbol, target_profit=TARGET_PROFIT_USD, max_loss=STOP_LOSS_USD):
    """
    Monitors open positions for EURUSD.
